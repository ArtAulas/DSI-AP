...
def buscarId(id:int, db:Session=Depends(get_db)):
    endereco_on_db=db.query(Endereco).filter(Endereco.id==id).first()
...
def atualizarId(id:int, request:EnderecoRequest, db:Session=Depends(get_db)):
    endereco_antigo=db.query(Endereco).filter(Endereco.id==id).first()
    if endereco_antigo is None:
        return Response(content='Endereço não encontrado',status_code=404)
    db.merge(Endereco(id=id, **request.model_dump()))
    db.commit()
    endereco_novo=db.query(Endereco).filter(Endereco.id==id).first()
